...
def run(resume=False):    
    ...
    if not resume:
        ...
        # Model
        net = model.PixelCNN(1, n_classes, 
                             args.n_features, 
                             args.n_layers, 
                             args.n_bins, 
                             args.dropout)
        ...
        if not (os.path.isfile(exp_dir/'params.json')
    ...
    trainer.generate_between_classes(net, [28, 28], [1, 7],
                             os.path.join(exp_dir,'1-7.jpeg'), n_classes, args.cuda)
    trainer.generate_between_classes(net, [28, 28], [3, 8],
                             os.path.join(exp_dir,'3-8.jpeg'), n_classes, args.cuda)
    trainer.generate_between_classes(net, [28, 28], [4, 9],
                             os.path.join(exp_dir,'4-9.jpeg'), n_classes, args.cuda)
    trainer.generate_between_classes(net, [28, 28], [5, 6],
                             os.path.join(exp_dir,'5-6.jpeg'), n_classes, args.cuda)
